# ============================================================
#  Module: College Talent Map (mod_map_college.R)
#  CSS styles moved to www/styles.css
# ============================================================
...
mod_map_college_server <- function(id, data, raw_data) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns

    us_map_data <- readRDS("us_map_data.rds")

    # ── University → state mapping ─────────────────────────────
    ...
    count_players <- function(df, state_filter = NULL) {
      ...
      if (!is.null(state_filter)) {
        base <- base %>% dplyr::filter(state_id == state_filter)
      } else {
        base <- base %>% dplyr::filter(!is.na(state_id))
      }
      base %>%
    ...
    output$college_map <- renderHighchart({
      ...
      highchart(type = "map") %>%
        ...
        hc_nba_title("USA Talent Map: College Impact by State") %>%
        hc_nba_subtitle("Click on a state to see university breakdown") %>%
        hc_tooltip(
    ...
    # ── Helper: generate NBA-styled table HTML ──────────────────
