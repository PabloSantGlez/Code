# ============================================================
#  Module: Pick Performance (mod_pick_performance.R)
#
#  Shows 3 lines for the selected year:
#    · overall_performance  — actual player performance
#    · observed_perf        — historical avg by pick position
#    · expected_perf        — spline curve (theoretical expected)
# ============================================================
    ...
    moduleServer(id, function(input, output, session) {
        output$chart_pick_perf <- renderHighchart({
            ...
            df <- data %>%
                ...
                distinct(
                    overall_pick, player, overall_performance,
            ...
            highchart() %>%
                ...
                hc_nba_title(paste0("Pick Performance — Draft ", current_year)) %>%
                hc_nba_subtitle("Player Performance vs. Historical Pick Performance & Expected Performance") %>%
                # ── Series ─────────────────────────────────────────────
                hc_add_series(
                    data = purrr::pmap(
